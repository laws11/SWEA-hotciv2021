...
public class CivDrawing implements Drawing, GameObserver {
  ...
  private Map<City, CityFigure> positionToCityFigureMap;
  ...
  public CivDrawing(DrawingEditor editor, Game game) {
    ...
    synchronizeCityFigureCollectionWithGameUnits();
  ...
  protected void synchronizeCityFigureCollectionWithGameUnits() {
    // iterate all tile positions and ensure that our figure
    // collection truthfully match that of game by adding/removing
    // figures.
    Position p;
    System.out.println("I called sync city");
    for (int r = 0; r < GameConstants.WORLDSIZE; r++) {
      for (int c = 0; c < GameConstants.WORLDSIZE; c++) {
        p = new Position(r, c);
        City city = game.getCityAt(p);
        CityFigure cityFigure = positionToCityFigureMap.get(city);
        // Synchronize each tile position with figure collection
        if (city != null) {
          // if a unit is present in game, then
          if (cityFigure == null) {
            ...
            // We add the figure both to our internal data structure
            positionToCityFigureMap.put(city, cityFigure);
            // and of course to MiniDraw's figure collection for
            // visual rendering
          ...
          // no unit at tile, maybe there is a unitFigure wrongly here
          if (cityFigure != null) {
            positionToCityFigureMap.remove(city);
  ...
  protected void synchronizeIconsWithGameState() {
    ...
    if (ageIcon == null) {
      ageIcon =
            new TextFigure(Math.abs(game.getAge()) + " BC",
  ...
  public void worldChangedAt(Position pos) {
    ...
    if (c != null) {
      ...
      positionToCityFigureMap.put(c, cf);
  ...
  public void turnEnds(Player nextPlayer, int age) {
    ...
    ageIcon.setText(Math.abs(age) + " BC");
    requestUpdate();
  }
  ...
  protected ImageFigure cityWorkFocusIcon;
  ...
  public void tileFocusChangedAt(Position position) {
    ...
    if(game.getCityAt(position) != null){
      ...
      cityShieldIcon = new HotCivFigure(player + "shield", new Point(GfxConstants.CITY_SHIELD_X, GfxConstants.CITY_SHIELD_Y), GfxConstants.CITY_TYPE_STRING);
      figureCollection.add(cityShieldIcon);
      cityProductionIcon =
              new HotCivFigure(game.getCityAt(position).getProduction(),
                      new Point(GfxConstants.CITY_PRODUCTION_X,
                              GfxConstants.CITY_PRODUCTION_Y), GfxConstants.UNIT_TYPE_STRING);
      figureCollection.add(cityProductionIcon);
      cityWorkFocusIcon = new ImageFigure(game.getCityAt(position).getWorkforceFocus(), new Point(GfxConstants.WORKFORCEFOCUS_X, GfxConstants.WORKFORCEFOCUS_Y));
      figureCollection.add(cityWorkFocusIcon);
    }
    if(game.getUnitAt(position) != null) {
      if(game.getUnitAt(position).getOwner().equals(Player.BLUE)){
        player = "blue";
      }
      unitShieldIcon = new HotCivFigure(player + "shield",
      ...
      unitMovesLeft =
              new TextFigure(String.valueOf(game.getUnitAt(position).getMoveCount()),
                      new Point(GfxConstants.UNIT_COUNT_X,
                              GfxConstants.UNIT_COUNT_Y));
      figureCollection.add(unitMovesLeft);
    }
